...
public class AIController {
    ...
    private AreaCell getDumbMovePosition(ObjActor selected_obj, AbilityActor move_action, int type) {
        ...
        area_computer.clearAreaList();
        if (type == 0 && initial_distance <= min_distance && com.icefill.game.utils.Randomizer.hitInRatio(.5f))
            return null;
        else return to_return;
    }


    //set Target to move position
    //return areaCell having obs to destroy
    private AreaCell findObjToDestroy() {
        ...
        while (!obs_stack.isEmpty()) {
            AreaCell cellNearObs = obs_stack.pop();
            for (int dir = 0; dir < 4; dir++) {
                ObjActor temp_obj = Global.getCurrentRoom().getNearObj(cellNearObs, dir);
                AreaCell cellHavingObs = Global.getCurrentRoom().getNearCell(cellNearObs, dir);
                if (temp_obj != null && temp_obj.isObstacle()) {
                    if (getMeleeMovePosition(selected_obj, cellHavingObs) != null) {
                        area_computer.setAreaListWithoutClearCount(area_computer.computeInspectionArea(Global.getSelectedObj()));
                        target = backtrackingUntilInRange(cellNearObs, action.targeting_max_range);
                        area_computer.clearAreaCount();
                        area_computer.clearAreaList();
                        return cellHavingObs;
                    }
    ...
    public AreaCell meleeAI(ObjActor selected_obj) {
        ...
        if (findMeleeActionAndTarget(selected_obj)) return target;
            ...
            if (target == null) {// no target, no move postion
                AreaCell obsToDestroy = findObjToDestroy();
                //Target should be areaCell position move to
                if (obsToDestroy != null && target != null) {
                    if (target.equals(Global.getCurrentRoom().getCell(selected_obj)) &&Global.getSelectedObj().job.ai_type.equals("melee") ) { // if melee char and
                        selected_obj.selectAction(selected_obj.getActionList().getFirst());
                        target = obsToDestroy;
                    } else if (Randomizer.hitInRatio(.3f)) {
                        target = getDumbMovePosition(selected_obj, (AbilityActor) selected_obj.getMoveAction().action, 1);
                        if (target==null) selected_obj.selectAction(null);
                    } else target = null;
    ...
    public AreaCell chooseObjAndActionAndTarget() {
        ...
        // Select Obj the function return true if selected char properly.
        if (selectObjtoAct() == false) return null;
        ObjActor selected_obj = Global.getSelectedObj();
        target = null;
        ...
        //should choose appropriate obj and action if target is not null
        return target;
