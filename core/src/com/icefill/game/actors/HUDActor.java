...
public class HUDActor extends Table{
 ...
 //   Table inventory_table;
    ...
    public HUDActor(){
		...
		battle_win_window = new BattleWinWindow(Assets.getSkin());
		...
		//top_table.pack();
		//top_table.add(information_table).expandX().center();
		status_parent_table.add(ability_table).left().pad(5);
		//status_parent_table.add(ability_table2).bottom();

		bottom_table.add(status_parent_table).expandX().left();
		//top_table.add(status_parent_table2).expandX().right().bottom();
		
		//information_table.add("BUMMER");
		//bottom_table.pack();
		...
		//bottom_table.setDebug(true,true);
	...
	public void showMap() {minimap_table.setVisible(true);}
	public void hideMap() {minimap_table.setVisible(false);}
	public Table getMinimap() {return minimap_table;}
	...
	public void addAbilitySubMethod(ObjActor selected_obj,ActionContainer temp,boolean is_on_battle)
	{
	...
	public void addAbility(ObjActor selected_obj,boolean is_on_battle)
	  {
		...
		if (selected_obj != null) {
	      ...
	      for (ActionContainer temp : selected_obj.getActionList()) {
	    	  ...
	    	  if (temp.action.getActionName().equals("Wait")){
	    		  ability_table.add(temp.action);
	    		  temp.addListener();
	    		  continue;
	    	  }
	    	  
	    	  addAbilitySubMethod(selected_obj,temp, is_on_battle);
	    	 //}
	      }
	      // itemm action
