...
public class ItemWindow extends BasicWindow {
	...
	public void setButton(SlotActor slot)
	{
		button_table.clear();
		...
		if (slot.type==-1)
		{
			button_table.add(drop_button);
		}
		else
		{
			button_table.add(un_equip_button);
			
		}
		if (slot.quick==null)
		{
			if (slot.equip.isDisposable())
			button_table.add(add_to_shortcut_button);
		}
		else
		{
			button_table.add(delete_shortcut_button);
		}
		if (slot.equip.status==null){
		 ...
		 if ( slot.equip.equip_action!=null)
		  {
			  //if (Global.getSelectedObj().isLearnable((AbilityActor)slot.equip.equip_action.action)){
				  button_table.add(memorize_button);
			 // }
	...
	public void setDescription(SlotActor slot)
	{
	...
	public void setSlot(SlotActor slot) {
		if (slot.equip!=null)
		{
			setButton(slot);
			setDescription(slot);
	...
	public void showTable(SlotActor slot, ObjActor obj) {
		setSlot(slot);
	...
	public class ItemDescriptionTable extends Table
	{
		...
		public void setDescription(SlotActor slot,Boolean has_alternative)
		{
