...
public class ObjSprites extends BasicSprites {
	...
	public void drawAnimation(Batch batch,float elapsed_time,int anim,int direction,float x,float y,float origin_x,float origin_y,float rotation,float scalex,float scaley,InventoryActor inventory,Color color) {
		if (!equippable) {
			...
			/*
			if (!sound_run && anim==WALK && index % 2 ==1) {
				
				foot1.play();
				sound_run=true;
			}
			if (sound_run && anim==WALK && index%2==0) {
				sound_run=false;
			}
			*/
			//Color base_color;
			//if (inventory.getEquip(1)!= null) base_color= inventory.getEquip(1).getBodyColor();
			//else base_color=Color.WHITE;
			//batch.setColor(base_color);
			batch.setColor(color);
			//batch.draw(animation[anim][direction][ARM2].getKeyFrame(elapsed_time,true), x-anchor_x,y-anchor_y);
			batch.setColor(1f,1f,1f,1f);
			...
			//batch.setColor(color);
			//batch.setColor(base_color);
			//batch.draw(animation[anim][direction][ARM1].getKeyFrame(elapsed_time,true), x-anchor_x, y-anchor_y);
			//batch.setColor(1f,1f,1f,1f);
//			if (direction == DL || direction == UR) {
			if (anim==GUARD) {
				if (inventory.getEquip(3) !=null && (direction==DL||direction==DR)) {
					...
					if (inventory.getEquip(3)!=null) {
						...
						case DL:
						case UR: shield_dir=DR; break;
						default: shield_dir=DL; break;
						}
	...
	//Assets.getFont().draw(batch,"hp:"+hp,getX(),getY()+40);
			...
			for (int ani=0;ani<N_ANIMATION;ani++){
				for (int dir=0;dir<N_DIRECTION;dir+=direction_adder) {
					for (int bodyparts=0;bodyparts<n_bodyparts;bodyparts++){
						...
						if (ani== DEAD||ani== RAISE) animation[ani][dir][bodyparts].setPlayMode(PlayMode.NORMAL); else
						...
						if (is_symmetric) {
							...
							if (ani== DEAD || ani==RAISE) animation[ani][dir+1][bodyparts].setPlayMode(PlayMode.NORMAL); else
